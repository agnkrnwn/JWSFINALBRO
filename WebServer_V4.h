...
void processMessage(String data) {
  ...
  // Date Setting
  if (payload[0] == 't' && length >= 9) {
    uint16_t tahun = ((payload[1]-'0')*1000) + ((payload[2]-'0')*100) +
                     ((payload[3]-'0')*10) + (payload[4]-'0');
    uint8_t bulan = ((payload[5]-'0')*10) + (payload[6]-'0');
    uint8_t tanggal = ((payload[7]-'0')*10) + (payload[8]-'0');
  ...
  // Time Setting
  if (payload[0] == 'j' && length >= 5) {
    uint8_t jam = ((payload[1]-'0')*10) + (payload[2]-'0');
    uint8_t menit = ((payload[3]-'0')*10) + (payload[4]-'0');
