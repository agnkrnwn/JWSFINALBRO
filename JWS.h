...
// Helper untuk Delay non-blocking agar WebPage tetap jalan
...
void AlarmSholat() {
  ...
  if (configjws.enable_tanbih == 1) {
    ...
    if (Hor == hours && Min == minutes) {
      ...
      smartDelay(1000);
      mp3.stop();
      smartDelay(configjws.durasiadzan);    
      Disp.clear();
    }
  }

  // Imsak (terpisah dari Tanbih)
  if (configjws.enable_imsak == 1) {
    get_float_time_parts(times[0], hours, minutes);
...
void TampilJadwalSholat() {
  ...
  uint8_t enableFlags[] = {configjws.enable_subuh, 0, configjws.enable_dzuhur, configjws.enable_ashar, 0, configjws.enable_maghrib, configjws.enable_isya};
  int koreksiFlags[] = {configjws.koreksi_subuh, 0, configjws.koreksi_dzuhur, configjws.koreksi_ashar, 0, configjws.koreksi_maghrib, configjws.koreksi_isya};
  int hours, minutes;  
  ...
  if (cM - pM >= 3000) {
    ...
    // Skip jika waktu sholat disabled
    if (enableFlags[i] == 0) {
      i++;
      if (i > 7) {
        // Tampilkan Tanbih jika enabled
        if (configjws.enable_tanbih == 1) {
          get_float_time_parts(times[0], hours, minutes);
          minutes = minutes + configjws.koreksi_tanbih;
          if (minutes < 11) {
    ...
    minutes = minutes + koreksiFlags[i];
